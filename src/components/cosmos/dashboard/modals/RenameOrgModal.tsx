...
export function RenameOrgModal({ current, onClose, onRename }) {
  ...
  return (
    <Modal onClose={onClose}><div className="modal-body"><div className="modal-eyebrow">{mo.eyebrow}</div><h3>{og.renameTitle}</h3>
      <label className="field-l" htmlFor={nameId}>{mo.nameLabel}</label><input id={nameId} className="field" value={name} onChange={(e) => setName(e.target.value)} placeholder={mo.namePlaceholder} autoFocus />
