...
import { fmtDateTime, notifIp, deviceLabel } from "@/components/cosmos/dashboard/helpers";
...
export function NotificationsMenu({ items, loading, error, unread = 0, onOpen, onViewAll }) {
  ...
  return (
    <div className="nav-user" ref={ref}>
      <button className={`icon-btn notif-bell${unread > 0 ? " has-unread" : ""}`} title={nt.title} aria-label={nt.title} aria-haspopup="menu" aria-expanded={open} onClick={toggle}>{DI.bell}{unread > 0 && <span className="notif-dot" />}</button>
      {open && (
